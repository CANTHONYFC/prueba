...
#2. CALENDAR INIT
#3. FORM VALIDATION
#4. DATE RANGE PICKER , DATE PICKER
#5. DATATABLES
#6. EDITABLE TABLES
...
//  });
  ...
  if ($('.select2').length) {
    $('select.select2').not(".ajax-remote").select2({placeholder:"Seleccione una opcion",language: "es",allowClear: true,width:'100%'});
    
  }
   if ($('.select-all').length) {
    $('select.select-all').not(".ajax-remote").select2({placeholder:"Todas las opciones",language: "es",allowClear: true,width:'100%'});
